...
names = re.search(r"MODEL_NAMES = \[(.*?)\]", open(D + 'js/assets.js').read(), re.S).group(1)
names = re.findall(r"'([^']+)'", names)
models = {n: base64.b64encode(open(D + 'models/' + n + '.glb', 'rb').read()).decode() for n in names}
...
html = html.replace('<script type="module" src="js/main.js"></script>',
                    '<script>window.__EMBED_MODELS = ' + json.dumps(models) + ';</script>\n<script type="module">\n' + js + '\n</script>')
